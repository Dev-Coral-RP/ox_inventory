...
local BackpackTiers = {
    ...
    epic   =  { slots = 40, weight = 50000 },
    -- legendary = { slots = 50, weight = 70000 },
}

local function hasExport(name)
    return type(exports.ox_inventory[name]) == 'function'
end

local function getSlotItem(src, slot)
    if hasExport('GetSlot') then
        return exports.ox_inventory:GetSlot(src, slot)
    end
    if hasExport('Inventory') then
        local inv = exports.ox_inventory:Inventory(src)
        return inv and inv.items and inv.items[slot] or nil
    end
    return nil
end

local function setSlotMetadata(src, slot, meta)
    if hasExport('SetMetadata') then
        return exports.ox_inventory:SetMetadata(src, slot, meta)
    end
    return false
end

local function registerStash(id, label, slots, maxWeight)
    if hasExport('RegisterStash') then
        return exports.ox_inventory:RegisterStash(id, label, slots, maxWeight, false)
    end
    TriggerEvent('ox_inventory:registerStash', id, label, slots, maxWeight)
end

-- Read tier from the instance metadata OR fall back to items.lua defaults
local function resolveTier(item)
    local tier = item?.metadata?.tier
    if tier then return tostring(tier):lower() end

    local defs = exports.ox_inventory:Items()
    local def = defs and defs[item.name]
    if def then
        if def.metadata and def.metadata.tier then
            return tostring(def.metadata.tier):lower()
        end
        if def.tier then
            return tostring(def.tier):lower()
        end
    end
    -- final fallback
    local byName = item.name:match('backpack_(%w+)')
    return (byName and byName:lower()) or 'common'
end

-- Ensure the bag has a unique id stored on the item instance
local function ensureBagMeta(src, slot)
    local item = getSlotItem(src, slot)
    if not item then return nil end

    item.metadata = item.metadata or {}

    if not item.metadata.bagid then
        item.metadata.bagid = ('bp:%s:%s'):format(item.name or 'bag', math.random(100000, 999999))
        if not setSlotMetadata(src, slot, item.metadata) then
            local count = item.count or 1
            exports.ox_inventory:RemoveItem(src, item.name, count, false, slot)
            exports.ox_inventory:AddItem(src, item.name, count, item.metadata, slot)
        end
    end
...
-- Open this backpack's stash with capacity based on tier
RegisterNetEvent('bp:server:open', function(slot)
    local src = source
    if not slot then return end

    local item = ensureBagMeta(src, slot)
    if not item then return end

    local tier = resolveTier(item)
    local cfg  = BackpackTiers[tier] or BackpackTiers.common

    local stashId = ('backpack:%s'):format(item.metadata.bagid)
    registerStash(stashId, item.label or 'Backpack', cfg.slots, cfg.weight)
